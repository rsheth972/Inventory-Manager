...
if __name__ == "__main__": 
    ...
    cur.execute("SELECT SUM(net_total),order_date FROM invoice GROUP BY order_date;")
    ...
    plt.title('Regression to predict the total sales based on the days')

    # plotting regression line 
    plot_regression_line(Y, X, b) 

    cur.execute("SELECT product_name,SUM(qty),price FROM invoice_details GROUP BY product_name;")
    ...
    textA = []
    for row in cur.fetchall() :
        X_bar.append(str(row[0]))
        # print(type(row[1]))
        Y_bar.append(int(row[1]))
        Z.append(int(row[2]))
        total.append(int(row[1])*int(row[2]))
        hii = "Qty :"+str(int(row[1]))+"<br>Price : "+str(int(row[2]))
        textA.append(hii)
    print(X_bar)
    print(Y_bar)
    print(Z)
    print(total)
    print(textA)
    plt.bar(X_bar, total, align='center', alpha=0.5)
    ...
    plt.bar(X_bar, Y_bar, align='center', alpha=0.5)
    plt.xlabel('Products')
    plt.ylabel('Sales')
    plt.title('Sales of products')
    plt.show()
